<template>
  <div>
    <v-layout>
      <v-flex>
        <v-dialog v-model="patientInfo.dialog" persistent max-width="600px" eager>
          <v-card>
            ...
            <v-card-text>
              ...
              <v-text-field v-model="form.allergies" label="Allergies"> </v-text-field>
              <v-text-field
...
export default {
  ...
  methods: {
    ...
    ...mapActions("disease", ["getDiseases", "updateDisease"]),

    close() {
      (this.patientInfo.dialog = false), (this.patientInfo.edit = false);
    },
    ...
    update(id) {
      ...
      if (this.form.last_disease_diagnosed) {
        for (var i in this.diseases) {
          if (this.diseases[i].name == this.form.last_disease_diagnosed) {
            this.diseases[i].current++;
            var disease = {
                form: this.diseases[i],
                id: this.diseases[i].id,
                district_id: this.diseases[i].district_id
            };
